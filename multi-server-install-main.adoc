...
NOTE: _Before the Main menu is displayed, the installer checks to see if the
...
3. Type `Y`, and press _Enter_ to modify the system. The selected packages are installed on the server.
...
** When `Save configuration data to file` appears, type `Yes` and press _Enter_.
** The next request asks where to save the files.
To accept the default, press _Enter_.
To save the files to another directory, enter the directory and press _Enter_.

12. When `The system will be modified - continue? [No]` appears, type `y` and press _Enter_.
+
--
...
2. Type `y` and press _Enter_ to install the *zimbra-logger* package (optional and only on one mail server) and *zimbra-store*.
...
3. Type `Y`, and press _Enter_ to modify the system.
...
+
--
NOTE: _By default, the domain name portions of the email addresses for the
...
NOTE: See the release notes for additional configuration information
...
+
--
NOTE: If you are installing the {product-short}-proxy package, see
...
If you are installing `zimbra-proxy` on the MTA server, select the `zimbra-proxy` package and the `zimbra-memcached` package. Follow the installation process for <<_installing_zimbra_mta_on_a_server>>.
...
* {product-name} ships a default `zimbra` user with a disabled
  ...
  but if you have modified spam rules to disallow any ssh access to
