...
* Zimbra Touch Client, Zimbra Mobile Client, and Zimbra HTML (Standard) Client are no longer a part of {product-name} beginning from Version 9.0.0.
...
** To know more about highlights of the {modern-client}, please refer to the *https://zimbra.github.io/zimbra-9-beta/Whats%20New%20in%20Zimbra%20MWC.pdf[What's New]* document for {product-name} {product-version}.
...
** Existing customized themes, logo branding changes, and crontab changes are incompatible with, and hence do not reflect in, the {modern-client}. Branding needs to be re-configured to work with the {modern-client}. The web client also does not currently support themes.
...
*** The Zimlets for Zimbra Drive and Zimbra Docs are available for both the web clients. You can install separate versions of these services for the {web-client} and the {modern-client}.
...
ifdef::z9[]
[IMPORTANT]
Branding needs to be re-configured to work with the {modern-client}. And the {modern-client} currently does not support themes.
endif::z9[]
...
ifdef::z9[]
[IMPORTANT]
Zimlets are supported on both the {web-client} and the {modern-client}. However, Zimlets used for Classic Web Client are not compatible with the {modern-client}. Currently, there is no way to migrate the zimlets from Classic to the {modern-client} or vice-versa. Existing zimlets need to be re-developed for them to work on the {modern-client}.
endif::z9[]
