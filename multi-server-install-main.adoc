...
scalable multi-server deployment. {product-short} IMAPD is normally installed
 separately from the mailbox server. {product-short} IMAPD can be installed on more than one server.

IMPORTANT: After installation the new server will have to be added to the `zimbraReverseProxyUpstreamImapServers` LDAP multi-value configuration in order to enable the new server to service IMAP connections.


=== Installing on the Mailbox Server


If you are installing `zimbra-imapd` on a mailbox server, select the `zimbra-imapd` package in addition to `zimbra-store`.
Follow the installation process for <<_installing_zimbra_mta_on_a_server>>.
After Step 11, configure Zimbra-imapd.
...
to open a SSH session to the server, log on to the server as
root, and unpack the {product-short} software.
. Select to install the `zimbra-imapd` package.
...
+
--
The imapd server hostname is displayed. You must change the LDAP
master host name and password to be the values configured on the LDAP
server.
...
** Type 2, press *Enter*, and the following menu displays:
