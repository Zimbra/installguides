...
hundred of thousands of users, and it's still a Beta Release. {product-short} recommends
...
* {product-short}-dnscache is recommended to be installed on the MTA Servers only when
is not another DNS Server like dnsmasq or bind running on the same Zimbra Server.
+
--
[source%nowrap]
....
