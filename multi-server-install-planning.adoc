...
5. Zimbra Proxy Route Lookup Handler/NLE locates the route information from
   LDAP for the account being accessed and returns this back to Zimbra Proxy.
...
The proxy also optionally connects to the IMAPD service on the Zimbra IMAPD ports
if IMAPD is installed and configured at the global level or for a specific set of mailbox servers.

*Zimbra Proxy Port Mapping*

[cols=",",options="header",]
|========================
2+|Zimbra Proxy Ports (External to {product-name})
|HTTP |80
|HTTPS |443
|POP3 |110
...
[cols=",",options="header",]
|========================
2+|Zimbra Mailbox Ports (Internal to {product-name})
...
[cols=",",options="header",]
|========================
2+|Zimbra IMAPD Ports (Internal to {product-name})
