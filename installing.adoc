...
To install {product-name} on a single server, enter `Y` for the ldap, logger, mta, snmp, store, and spell packages.
If you use IMAP/POP Proxy, enter `Y` for the Zimbra proxy package and the Zimbra Memcached package.
...
5. Next, type `Y` and press kbd:[Enter] to modify the system.
...
11. Select the following from the store configuration menu:
    ...
    * Type `4` to set the Admin Password.
...
14. If no other defaults need to be changed, type `a` to apply the configuration changes.
Press kbd:[Enter]
+
--
----
