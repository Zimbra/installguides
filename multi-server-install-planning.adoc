...
This chapter describes the components that are installed and reviews the configuration options that you can make when installing {product-name}.
...
NOTE: The {product-short} Proxy package does not act as a firewall and needs to be behind
...
NOTE: The following ports are used either by {product-short} Proxy or by {product-short} Mailbox
