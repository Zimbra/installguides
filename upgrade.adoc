...
--
+
5. If you were using `zmlogger` prior to {product-abbrev} 8.0.7, it is possible that numerous `rdd` files could be generated causing large amounts of disk space to be used.
...
If you do not want to upgrade, but prefer to install {product-name} {product-version} as a new installation, when you run the {product-name} {product-version} install script, enter `N` when asked `Do you wish to upgrade?`
