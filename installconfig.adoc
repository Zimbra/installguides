...
| Toggle whether **zimbra-convertd** is enabled or not - defaults to **yes** +
  ...
  on size of {product-short} environment. +
...
NOTE: New installs of {product-short} limit spam/ham training to the first MTA installed.
...
passes IMAP[S]/POP[S]/HTTP[S] client requests to other internal {product-short} services.
