...
. Run {product-short} X Webclient on any of the mailbox servers
	...
	* Create a production build by executing `npm run build`, if someone wants to use a different client or a customised client directory then it should be specified by using `CLIENT=client_name` before the command to make sure customisations are included in build
