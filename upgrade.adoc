...
upgrade. After you review the tasks in this section, go to <<Upgrade Instructions>>. Be sure to read the
...
NOTE: {product-name} {product-version} is a beta release, it is not recommended to install or upgrade this release in a production environment. The beta release is for demo and evaluation purposes. 

Please review the following information to decide if {product-name} {product-version}
is suitable for you.

* Zimbra Touch Client, Zimbra Mobile Client, and Zimbra HTML (Standard) Client are no longer a part of Zimbra Collaboration V9.0.0 (Beta). IMAPD, OpenDrive, and Talk are being deprecated, though they are available in the Beta release; they won’t be available in the final GA release.
...
* {product-name} {product-version} currently does not support rolling upgrades.
* {product-name} {product-version} introduces two versions of {product-short} Web Client -- Modern and Classic.
** A {product-short} license is required to use the Modern Web Client in {product-name}'s {product-edition-commercial}.
** It is recommended that {modern} is set as the default web client. Even after selecting a default, {modern} and {web-client} can be used simultaneously by users as per their choice.
** To know more about highlights of the Modern Web Client, please refer to the *https://zimbra.github.io/zimbra-9-beta/Whats%20New%20in%20Zimbra%20MWC.pdf[What's New]* document for {product-name} {product-version}.
** Classic Web Client offers the same functionalities as offered by the Advanced Web Client in  {product-name} 8.8.15 version.
** Modern Web Client does not currently support features like Personas, Briefcase, and Dumpster.
// Some of these features are part of upcoming releases.
// To view the detailed comparison of features supported in Modern Web Client, please refer to *Web Client Comparison* document.
** Existing customized themes, logo branding changes, and crontab changes are incompatible with, and hence do not reflect in, the Modern Web Client. Branding needs to be re-configured to work with the Modern Web Client. The web client also does not currently support themes.
** Currently supported languages in the application UI for the {modern} are - Chinese (China), English (US), French (France), German, Hindi, Indonesian, Italian, Japanese, Portuguese (Portugal), Spanish and Thai. 
** Zimlets are supported on both the Web Clients. 
*** Existing Zimlets work with Classic Web Client; however, they are incompatible with Modern Web Client. Currently, there is no way to migrate the zimlets from Classic to Modern Web Client or vice-versa. 
*** Zimbra Drive zimlet and Zimbra Docs zimlet, are available for both the web clients. You can install separate versions of these services for Classic Web Client and Modern Web Client.

You might need to perform the following tasks before you upgrade.
After you review the tasks in this section, please go to <<Upgrade Instructions>>. Be sure to read the
https://zimbra.github.io/zimbra-9-beta/release-notes.html[release notes]
information before upgrading.
endif::z9[]
...
If upgrading to {product-abbrev} 8.7.0 and above, you need to completely disable SSLv3
...
a time, following the instructions under <<_process_2>> below.
  ...
  . Proxy servers
  . Mailstore servers
  . Optionally install any new IMAPD servers
...
themes, logo branding changes, and crontab changes.  

ifdef::z9[]
[IMPORTANT]
Branding needs to be re-configured to work with the Modern Web Client. And the Modern Web Client currently does not support themes.
endif::z9[]
...
ifdef::z9[]
[IMPORTANT]
Zimlets are supported on both Classic and Modern Web Clients. However, Zimlets used for Classic Web Client are not compatible with Modern Web Client. Currently, there is no way to migrate the zimlets from Classic to Modern Web Client or vice-versa. Existing zimlets need to be re-developed for them to work on Modern Web Client.
endif::z9[]
