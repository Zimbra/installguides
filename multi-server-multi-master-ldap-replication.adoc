...
You can run the {product-short} multiple master CLI,
...
* The first master LDAP server ID is set to kbd:[1].
...
LDAP servers. You run the {product-short} install program on the
...
To find these passwords, on the {product-short} server run:
...
. Type kbd:[Y] and press kbd:[Enter] to install the `zimbra-ldap` package.
. Type kbd:[Y], and press kbd:[Enter] to modify the system. The selected packages are
installed. The Main menu shows the default entries for the LDAP
server.
. Type kbd:[1] to display the _Common Configuration_ submenu.
.. Type kbd:[2] to change the *LDAP Master host* name to the name of the
primary master's hostname; e.g., _master-1.example.com_.
.. Type kbd:[4] to change the *LDAP admin password* to the {product-short} admin
password of the primary master.
.. Type kbd:[r] to return to the main menu.
. Type kbd:[2] to display the _LDAP configuration_ submenu.
.. Type kbd:[4] to change the type to `mmr`.
+
NOTE: Item kbd:[5], _LDAP Server ID_, is set to kbd:[2]. If this is the second
master, leave it unchanged. If it the third or later master, select kbd:[5]
...
+
.. Type kbd:[7] to change the _LDAP replication password_.
.. Type kbd:[8] to change the _LDAP postfix password_.
.. Type kbd:[9] to change the _LDAP amavis password_.
.. Type `10` to change the _LDAP NGINX password_.
.. Type kbd:[r] to return to the main menu.
. Type kbd:[a] to apply the configuration changes. Press kbd:[Enter] to save
the configuration data.
. When *Save Configuration data to a file* appears, press kbd:[Enter].
. When *The system will be modified - continue?* appears, type kbd:[y] and press
kbd:[Enter].
+
...
kbd:[Enter]. The installation is complete.
...
In an existing {product-short} setup where there is already a single master and
...
. Assign the next Server ID to this master. This example is kbd:[3]
+
----
...
NOTE: Deleting an MMR node can only be performed in {product-name} 8.0.7 and later.
...
IMPORTANT: This script must be run on a {product-short} server that has a
