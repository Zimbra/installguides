...
{product-short} installation options. The LDAP replica server
...
server. You run the {product-short} install program on the replica
...
. Type kbd:[Y] and press kbd:[Enter] to install the `zimbra-ldap` package. In the
...
----
+
. Type kbd:[Y], and press kbd:[Enter] to modify the system. The selected packages
are installed. The Main menu shows the default entries for the LDAP
replica server. To expand the menu type kbd:[X] and press kbd:[Enter].
+
----
Main menu
...
----
+
. Type kbd:[1] to display the _Common Configuration_ submenus.
+
----
...
Master LDAP host.
. Type kbd:[3], to change the *Ldap port* to the same port as configured
for the Master LDAP server.
. Type kbd:[4] and change the *Ldap Admin password* to the
Master LDAP admin password, then type kbd:[r] to return to the main menu.
...
----
+
.. Type kbd:[2] and change *Create Domain* to `no`.
.. Type kbd:[4] for *LDAP replication password* and enter the same password to
...
====
+
. When the LDAP server is configured, type kbd:[a] to apply the
