...
* Zimbra Touch Client, Zimbra Mobile Client, Zimbra HTML (Standard) Client have been removed from {product-name} {product-version}
* {product-name} {product-version} currently does not support Rolling Upgrades
* {product-name} {product-version} introduces two versions of {product-short} Web Client, namely, Modern and Classic
** A {product-short} license is required in order to use the Modern Web Client in
...
** Modern Web Client does not currently support some features like Personas, Briefcase and Dumpster. Some of these features will be added during upcoming releases. To view the detailed comparison of features supported in Modern Web Client, please refer to *Web Client Comparison* document
** Existing customized themes, logo branding changes, and crontab changes will not work with Modern Web Client. Branding needs to be re-configured to work with the Modern Web Client. Themes are currently not supported for Modern Web Client.
** Zimlets are supported on both the Web Clients. 
*** Existing Zimlets will continue to work in the Classic Web Client, however they will not work on Modern Web Client. There is no way to migrate the zimlets from Classic to Modern Web Client or vice-versa. 
*** Zimbra Connect zimlet, Zimbra Drive zimlet and Zimbra Docs zimlet, are available for both the web clients. Seperate versions of these services need to be installed for Classic Web Client and Modern Web Client
...
ifdef::z9[]
[IMPORTANT]
Branding needs to be re-configured to work with the Modern Web Client. Themes are currently not supported on Modern Web Client.
endif::z9[]
...
ifdef::z9[]
[IMPORTANT]
Zimlets are supported on both Classic and Modern Web Clients. However, Zimlets used for Classic Web Client are not compatible with Modern Web Client. There is no way to migrate the zimlets from Classic to Modern Web Client or vice-versa. Existing zimlets need to be re-developed in order for them to work on Modern Web Client.
endif::z9[]
