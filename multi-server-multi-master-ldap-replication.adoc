...
. Type `Y` and press kbd:[Enter] to install the `zimbra-ldap` package.
. Type `Y`, and press kbd:[Enter] to modify the system. The selected packages are
...
.. Type `r` to return to the main menu.
. Type `a` to apply the configuration changes. Press kbd:[Enter] to save
